...
struct Nonce {
  ...
  std::vector<unsigned char> get() {
    return std::vector<unsigned char>(bz.begin(), bz.end());
  }
...
public:
  static auto make_secret_connection(std::vector<unsigned char>& loc_priv_key,
  ...
  auto derive_secrets(noir::Bytes32& dh_secret) -> std::vector<unsigned char>;
...
public:
  ...
  std::vector<unsigned char> loc_pub_key;
  std::vector<unsigned char> loc_signature;

  std::vector<unsigned char> rem_pub_key;
...
private:
  std::vector<unsigned char> loc_priv_key;
  ...
  std::unique_ptr<std::vector<unsigned char>> recv_buffer;
  std::size_t read_pos;
