...
static void strobe128_run_f(MerlinStrobe128* ctx) {
...
static void strobe128_absorb(MerlinStrobe128* ctx, const uint8_t* data, size_t data_len) {
  size_t i;
...
static void strobe128_squeeze(MerlinStrobe128* ctx, uint8_t* data, size_t data_len) {
  size_t i;
  for (i = 0; i < data_len; ++i) {
...
static void strobe128_begin_op(MerlinStrobe128* ctx, uint8_t flags, uint8_t more) {
  if (more) {
...
static void strobe128_meta_ad(MerlinStrobe128* ctx, const uint8_t* data, size_t data_len, uint8_t more) {
  strobe128_begin_op(ctx, FLAG_M | FLAG_A, more);
  strobe128_absorb(ctx, data, data_len);
}

static void strobe128_ad(MerlinStrobe128* ctx, const uint8_t* data, size_t data_len, uint8_t more) {
  strobe128_begin_op(ctx, FLAG_A, more);
  strobe128_absorb(ctx, data, data_len);
}

static void strobe128_prf(MerlinStrobe128* ctx, uint8_t* data, size_t data_len, uint8_t more) {
  strobe128_begin_op(ctx, FLAG_I | FLAG_A | FLAG_C, more);
  strobe128_squeeze(ctx, data, data_len);
}

static void strobe128_init(MerlinStrobe128* ctx, const uint8_t* label, size_t label_len) {
  uint8_t init[18] = {1, 168, 1, 0, 1, 96, 83, 84, 82, 79, 66, 69, 118, 49, 46, 48, 46, 50};
...
void merlin_transcript_commit_bytes(
  MerlinTranscript* mctx, const uint8_t* label, size_t label_len, const uint8_t* message, size_t message_len) {
