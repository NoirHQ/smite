...
inline rocksdb::Slice to_slice(const bytes& v) {
  return {v.data(), v.size()};
}

inline bytes to_bytes(const rocksdb::Slice& v) {
...
namespace detail {
  ...
  struct value_storage<T*> {
    using type = T;
    constexpr T* as_ptr(T* value) const {
      return (value);
    }
  };
  ...
  template<typename T, std::size_t N>
  auto insert_key(noir::db::session::shared_bytes& dest, size_t index, T value) {
...
template<typename T, std::size_t N>
auto insert_key(noir::db::session::shared_bytes& dest, size_t index, std::array<T, N> value)
  -> std::enable_if_t<std::is_unsigned_v<T>, void> {
  detail::insert_key<T*, N>(dest, index, value.data());
}

inline void insert_key(noir::db::session::shared_bytes& dest, size_t index, float64_t value) {
  auto float_key = detail::float_to_key<uint64_t>(value);
  detail::insert_key<uint64_t, 1>(dest, index, float_key);
}

inline void insert_key(noir::db::session::shared_bytes& dest, size_t index, float128_t value) {
  auto float_key = detail::float_to_key<detail::uint128_t>(value);
  // underlying storage is implemented as uint64_t[2], but it is laid out like it is uint128_t
...
public:
  ...
  int64_t first_revision() const {
    return state.revision - state.undo_stack.size();
  }
  ...
  void undo(bool write_now = true) {
    ...
    while (rocks_it->Valid()) {
      ...
      while (ds.remaining()) {
        ...
        if (old_value)
          check(
            batch.Put({key, key_size}, {old_value, old_value_size}), "undo_stack::undo: rocksdb::WriteBatch::Put: ");
        else
...
struct write_session {
  ...
  void set(bytes&& k, const rocksdb::Slice& v) {
    ...
    if (stat.IsNotFound()) {
      auto [it, b] = cache.insert(cache_map::value_type{std::move(k), cached_value{0, nullptr, to_shared_bytes(v)}});
      changed(it);
      return;
    }
    ...
    if (compare_blob(v, orig_v)) {
      auto [it, b] =
        cache.insert(cache_map::value_type{std::move(k), cached_value{0, to_shared_bytes(orig_v), to_shared_bytes(v)}});
      changed(it);
    } else {
  ...
  void erase(bytes&& k) {
    ...
    auto [it, b] = cache.insert(cache_map::value_type{std::move(k), cached_value{1, to_shared_bytes(orig_v), nullptr}});
    changed(it);
  }
  ...
  struct iterator_impl {
    ...
    void lower_bound(const char* key, size_t size) {
      auto x = compare_blob(rocksdb::Slice{key, size},
        rocksdb::Slice{prefix.data() + hidden_prefix_size, prefix.size() - hidden_prefix_size});
      if (x < 0) {
    ...
    std::optional<key_value> get_kv() {
      ...
      return key_value{
        rocksdb::Slice{cache_it->first.data() + hidden_prefix_size, cache_it->first.size() - hidden_prefix_size},
...
public:
  ...
  public:
    ...
    void lower_bound(const bytes& key) {
      lower_bound(key.data(), key.size());
    }
...
FC_REFLECT(noir::db::chain_kv::undo_state, (format_version)(revision)(undo_stack)(next_undo_segment))
