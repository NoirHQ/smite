...
void undo_stack<Session>::commit(int64_t revision) {
  ...
  for (int64_t i = start_index; i >= 0; --i) {
    m_sessions[i].commit();
  }
...
void undo_stack<Session>::open() {
  ...
  if (fc::exists(undo_stack_dat)) {
    try {
      ...
      if (totem != undo_stack_magic_number)
        throw std::runtime_error(fmt::format("Undo stack data file '{}' has unexpected magic number: {}, Expected {}",
          undo_stack_dat.generic_string(), totem, undo_stack_magic_number));

      // validate version
      uint32_t version = 0;
      fc::raw::unpack(ds, version);
      if (version < undo_stack_min_supported_version || version > undo_stack_max_supported_version)
        throw std::runtime_error(fmt::format("Unsupported version of Undo stack data file '{}'. "
                                             "Undo stack data version is {} while code supports version(s) [{},{}]",
          undo_stack_dat.generic_string(), version, undo_stack_min_supported_version,
          undo_stack_max_supported_version));

      int64_t rev;
      fc::raw::unpack(ds, rev);

      size_t num_sessions;
      fc::raw::unpack(ds, num_sessions);
      for (size_t i = 0; i < num_sessions; ++i) {
        push();
        auto& session = m_sessions.back();

        size_t num_updated_keys;
        fc::raw::unpack(ds, num_updated_keys);
        ...
        size_t num_deleted_keys;
        fc::raw::unpack(ds, num_deleted_keys);
