...
shared_bytes::shared_bytes(const T* data, size_t size)
  : m_size{size * sizeof(T)}, m_offset{noir::db::session::details::aligned_size(m_size) - m_size}, m_data{[&]() {
      if (!data || size == 0) {
      ...
      auto result = std::shared_ptr<underlying_type_t>{
        new underlying_type_t[actual_size], std::default_delete<underlying_type_t[]>()};
...
inline shared_bytes::shared_bytes(size_t size)
  : m_size{size}, m_offset{noir::db::session::details::aligned_size(m_size) - m_size}, m_data{[&]() {
      if (size == 0) {
        return std::shared_ptr<char>{};
      }
...
inline shared_bytes shared_bytes::next() const {
  ...
  if (buffer.empty())
    throw std::runtime_error("shared_bytes::next() result buffer is empty");
  return noir::db::session::shared_bytes(buffer.data(), buffer.size());
}

inline size_t shared_bytes::size() const {
  return m_size;
}
inline size_t shared_bytes::aligned_size() const {
  return noir::db::session::details::aligned_size(m_size);
}
inline char* shared_bytes::data() {
  return m_data.get();
}
inline const char* const shared_bytes::data() const {
  return m_data.get();
}

inline bool shared_bytes::empty() const {
  return m_size == 0;
}
...
inline bool shared_bytes::operator!() const {
  return *this == shared_bytes{};
}

inline shared_bytes::operator bool() const {
  return *this != shared_bytes{};
}
...
inline shared_bytes shared_bytes::truncate_key(const shared_bytes& key) {
  if (key.empty())
    throw std::runtime_error("shared_bytes::truncate_key() invalid key parameter: empty");
...
template<typename Iterator_traits>
shared_bytes::shared_bytes_iterator<Iterator_traits>::shared_bytes_iterator(
  char* buffer, int64_t begin, int64_t end, int64_t index)
...
template<typename Iterator_traits>
shared_bytes::shared_bytes_iterator<Iterator_traits>& shared_bytes::shared_bytes_iterator<Iterator_traits>::operator+=(
  int64_t offset) {
...
template<>
struct less<noir::db::session::shared_bytes> {
  bool operator()(const noir::db::session::shared_bytes& lhs, const noir::db::session::shared_bytes& rhs) const {
    return lhs < rhs;
  };
};

template<>
struct greater<noir::db::session::shared_bytes> {
  bool operator()(const noir::db::session::shared_bytes& lhs, const noir::db::session::shared_bytes& rhs) const {
    return lhs > rhs;
  };
};
...
template<>
struct hash<noir::db::session::shared_bytes> {
  size_t operator()(const noir::db::session::shared_bytes& b) const {
    if (b.size() == 0) {
      return 0;
    }
...
template<>
struct equal_to<noir::db::session::shared_bytes> {
  bool operator()(const noir::db::session::shared_bytes& lhs, const noir::db::session::shared_bytes& rhs) const {
    return lhs == rhs;
  }
};
