...
void perform_session_level_test(const std::string& dbpath, bool always_undo = false) {
  ...
  auto root_session = noir::db::session_tests::make_session(dbpath);
  using session_type = noir::db::session::session<decltype(root_session)>;
...
TEST_CASE("session_create_test", "session_tests") {
  {
    auto session1 = noir::db::session_tests::make_session("/tmp/session18");
    make_data_store(session1, char_key_values, string_t{});
    verify_equal(session1, char_key_values, string_t{});
  }
  {
    auto session2 = noir::db::session_tests::make_session("/tmp/session19");
    make_data_store(session2, int_key_values, int_t{});
    verify_equal(session2, int_key_values, int_t{});
  }
}

TEST_CASE("session_rwd_test", "session_tests") {
  {
    auto session1 = noir::db::session_tests::make_session("/tmp/session20");
    make_data_store(session1, char_key_values, string_t{});
    for (const auto& kv : char_batch_values) {
      verify_rwd(session1, kv.first, kv.second);
    }
  }
  {
    auto session2 = noir::db::session_tests::make_session("/tmp/session21");
    make_data_store(session2, int_key_values, int_t{});
    for (const auto& kv : int_batch_values) {
      verify_rwd(session2, kv.first, kv.second);
    }
  }
}

TEST_CASE("session_rwd_batch_test", "session_tests") {
  {
    auto session1 = noir::db::session_tests::make_session("/tmp/session1");
    make_data_store(session1, char_key_values, string_t{});
    verify_rwd_batch(session1, char_batch_values);
  }
  {
    auto session2 = noir::db::session_tests::make_session("/tmp/session2");
    make_data_store(session2, int_key_values, int_t{});
    verify_rwd_batch(session2, int_batch_values);
  }
}

TEST_CASE("session_rw_ds_test", "session_tests") {
  {
    auto session1 = noir::db::session_tests::make_session("/tmp/session3");
    auto session2 = noir::db::session_tests::make_session("/tmp/session4");
    make_data_store(session1, char_key_values, string_t{});
    verify_read_from_datastore(session1, session2);
  }
  {
    auto session3 = noir::db::session_tests::make_session("/tmp/session5");
    auto session4 = noir::db::session_tests::make_session("/tmp/session6");
    make_data_store(session3, int_key_values, int_t{});
    verify_write_to_datastore(session3, session4);
  }
}

TEST_CASE("session_iterator_test", "session_tests") {
  {
    auto session1 = noir::db::session_tests::make_session("/tmp/session7");
    make_data_store(session1, char_key_values, string_t{});
    verify_iterators(session1, string_t{});
  }
  {
    auto session2 = noir::db::session_tests::make_session("/tmp/session8");
    make_data_store(session2, char_key_values, string_t{});
    verify_iterators<decltype(session2)>(session2, string_t{});
  }
  {
    auto session3 = noir::db::session_tests::make_session("/tmp/session9");
    make_data_store(session3, int_key_values, int_t{});
    verify_iterators(session3, int_t{});
  }
  {
    auto session4 = noir::db::session_tests::make_session("/tmp/session10");
    make_data_store(session4, int_key_values, int_t{});
    verify_iterators<decltype(session4)>(session4, int_t{});
  }
}

TEST_CASE("session_iterator_key_order_test", "session_tests") {
  {
    auto session1 = noir::db::session_tests::make_session("/tmp/session11");
    make_data_store(session1, char_key_values, string_t{});
    verify_session_key_order(session1);
  }
  {
    auto session2 = noir::db::session_tests::make_session("/tmp/session12");
    make_data_store(session2, char_key_values, string_t{});
    verify_session_key_order<decltype(session2)>(session2);
  }
  {
    auto session3 = noir::db::session_tests::make_session("/tmp/session13");
    make_data_store(session3, int_key_values, int_t{});
    verify_session_key_order(session3);
  }
  {
    auto session4 = noir::db::session_tests::make_session("/tmp/session14");
    make_data_store(session4, int_key_values, int_t{});
    verify_session_key_order<decltype(session4)>(session4);
  }
}

TEST_CASE("session_level_test_undo_sometimes", "session_tests") {
  noir::db::session_tests::perform_session_level_test("/tmp/session22");
}

TEST_CASE("session_level_test_undo_always", "session_tests") {
  noir::db::session_tests::perform_session_level_test("/tmp/session23", true);
}

TEST_CASE("session_level_test_attach_detach", "session_tests") {
  size_t key_count = 10;
  auto root_session = noir::db::session_tests::make_session("/tmp/session15");
  using session_type = noir::db::session::session<decltype(root_session)>;
  ...
  for (size_t i = 0; i < 3; ++i) {
    ...
    for (size_t j = 0; j < 3; ++j) {
      ...
      verify_equal(
        transaction_sessions.back(), collapse({root_session_kvs, kvs, transaction_session_kvs.back()}), int_t{});
...
TEST_CASE("session_overwrite_key_in_child", "session_tests") {
  auto verify_key_value = [](auto& ds, uint16_t key, uint16_t expected_value) {
    auto key_ = noir::db::session::shared_bytes(&key, 1);
    auto value = noir::db::session::shared_bytes(&expected_value, 1);
    auto value_read = ds.read(key_);
    CHECK(value_read == value);
  ...
  auto root_session = noir::db::session_tests::make_session("/tmp/session16");
  using session_type = noir::db::session::session<decltype(root_session)>;
  ...
  auto transaction_session_kvs = std::unordered_map<uint16_t, uint16_t>{
    {0, 2000}, {1, 2001}, {2, 2002}, {3, 2003}, {4, 2004}, {9, 2005}, {10, 2006}};
...
TEST_CASE("session_delete_key_in_child", "session_tests") {
  auto verify_keys_deleted = [](auto& ds, const auto& keys) {
    for (const uint16_t& key : keys) {
      auto key_ = noir::db::session::shared_bytes(&key, 1);
      CHECK(!ds.read(key_).has_value());
      CHECK(ds.find(key_) == std::end(ds));
      CHECK(ds.contains(key_) == false);
    }
    ...
    do {
      if (it != end) {
        ...
        CHECK(keys.find(*reinterpret_cast<const uint16_t*>(buffer.data())) == std::end(keys));
      }
      ++it;
    } while (it != begin);
  };
  ...
  auto verify_keys_exist = [](auto& ds, const auto& key_values) {
    for (const auto& key_value : key_values) {
      auto key = noir::db::session::shared_bytes(&key_value.first, 1);
      auto value = noir::db::session::shared_bytes(&key_value.second, 1);
      CHECK(*ds.read(key) == value);
      CHECK(ds.find(key) != std::end(ds));
    ...
    do {
      ...
      buffer = std::vector<shared_bytes::underlying_type_t>{std::begin(*key_value.second), std::end(*key_value.second)};
      auto value = *reinterpret_cast<const uint16_t*>(buffer.data());

      auto kv_it = key_values.find(key);
      if (kv_it != std::end(key_values)) {
        CHECK(value == kv_it->second);
        ++found;
      }

      ++it;
    } while (it != begin);
    CHECK(found == key_values.size());
  };
  ...
  auto root_session = noir::db::session_tests::make_session("/tmp/session17");
  using session_type = noir::db::session::session<decltype(root_session)>;
  auto root_session_kvs = std::unordered_map<uint16_t, uint16_t>{
    {0, 10}, {1, 9}, {2, 8}, {3, 7}, {4, 6}, {5, 5}, {6, 4}, {7, 3}, {8, 2}, {9, 1}, {10, 0}};
  write(root_session, root_session_kvs);
...
// TEST_CASE("session_iteration", "session_tests") {
...
//     auto cache_ds = noir::db::session::make_cache(memory_allocator);
//     auto rocksdb = make_rocks_db("testdb");
//     auto rocks_ds = noir::db::session::make_rocks_data_store(std::move(rocksdb), std::move(memory_allocator));

//     auto root_session = noir::db::session::make_session(rocks_ds, cache_ds);
...
//     auto block_session = noir::db::session::make_session(root_session);
//     write(block_session, block_session_kvs);

//     auto transaction_session_kvs = generate_kvs(5000);
//     auto transaction_session = noir::db::session::make_session(block_session);
//     write(transaction_session, transaction_session_kvs);
