...
public:
  ...
  const std::pair<std::vector<std::pair<shared_bytes, shared_bytes>>, std::unordered_set<shared_bytes>> read(
    const Iterable& keys);
...
protected:
  template<typename Iterable>
  const std::pair<std::vector<std::pair<shared_bytes, shared_bytes>>, std::unordered_set<shared_bytes>> read_(
    const Iterable& keys);
...
inline session<rocksdb_t>::session(std::shared_ptr<rocksdb::DB> db, size_t max_iterators)
  : m_db{[&]() {
      if (!db)
        throw std::runtime_error("db parameter cannot be null");
      return std::move(db);
    }()},
...
inline std::unordered_set<shared_bytes> session<rocksdb_t>::deleted_keys() const {
  return {};
}
...
inline bool session<rocksdb_t>::is_deleted(const shared_bytes& key) const {
  return false;
}
...
inline rocksdb::WriteOptions& session<rocksdb_t>::write_options() {
  return m_write_options;
}

inline const rocksdb::WriteOptions& session<rocksdb_t>::write_options() const {
  return m_write_options;
}

inline rocksdb::ReadOptions& session<rocksdb_t>::read_options() {
  return m_read_options;
}

inline const rocksdb::ReadOptions& session<rocksdb_t>::read_options() const {
  return m_read_options;
}

inline std::shared_ptr<rocksdb::ColumnFamilyHandle>& session<rocksdb_t>::column_family() {
...
session<rocksdb_t>::rocks_iterator<Iterator_traits>::rocks_iterator(
  session<rocksdb_t>& session, rocksdb::Iterator& rit, int64_t index)
