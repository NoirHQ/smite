...
public:
  ...
  auto flush() -> boost::asio::awaitable<Result<void>> {
    ...
    boost::asio::const_buffer buffers(buf.data(), n);
    auto res = co_await w->write(buffers);
    if (res.has_error()) {
      err = res.error();
    } else {
  ...
  auto write(std::vector<unsigned char>& p) -> boost::asio::awaitable<Result<std::size_t>> {
    std::span<unsigned char> bytes_s{p};
    std::size_t total_written{0};
    while (bytes_s.size() > available() && !err) {
      std::size_t remaining = available();
      std::size_t written = 0;
      if (n == 0) {
        boost::asio::const_buffer buffers(bytes_s.data(), remaining);
        auto res = co_await w->write(buffers);
        if (res.has_error()) {
          err = res.error();
        } else {
