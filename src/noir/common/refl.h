...
template<size_t I, typename T>
using FieldType = typename Field<I, T>::type;
...
template<size_t I, typename T>
const FieldType<I, T>& get(const T&);
...
template<typename T>
using BaseType = typename Base<T>::type;

/// \brief iterates fields of T
template<size_t I = 0, typename F, typename T>
bool for_each_field(F&& f, T& v) {
  if constexpr (!I && !std::is_void_v<BaseType<T>>) {
    if (!for_each_field(f, (BaseType<T>&)v)) {
      return false;
    }
  }
...
bool for_each_field(F&& f, const T& v) {
  if constexpr (!I && !std::is_void_v<BaseType<T>>) {
    if (!for_each_field(f, (const BaseType<T>&)v)) {
      return false;
    }
  }
  if constexpr (I < fields_count_v<T>) {
...
#define NOIR_REFLECT_FIELD_GET(r, TYPE, INDEX, FIELD) \
  namespace noir::refl { \
    ...
    template<> \
    inline const FieldType<INDEX, TYPE>& get<INDEX>(const TYPE& v) { \
      return v.FIELD; \
    } \
  }
