...
TEST_CASE("[jmt] insert_to_empty_tree", "[jmt]") {
  ...
  auto [_new_root_hash, batch] = *tree.batch_put_value_sets({{std::make_pair(key, value)}}, std::nullopt, 0);
  CHECK(!batch.stale_node_index_batch.size());
...
TEST_CASE("[jmt] insert_to_pre_genesis", "[jmt]") {
  ...
  auto [_root_hash, batch] = *tree.batch_put_value_sets({{std::make_pair(key2, value2)}}, std::nullopt, 0);
