...
private:
  ...
  static buffer_type* malloc() {
    return static_cast<buffer_type*>(pool_type::malloc());
  }
  static void free(buffer_type* ptr) {
    pool_type::free(ptr);
  }
...
public:
  ...
  inline bool read(char* d, size_t s) {
    ...
    throw_datastream_range_error("read", mb.bytes_to_read(), s - mb.bytes_to_read());
  }
...
class mb_peek_datastream {
public:
  explicit mb_peek_datastream(const message_buffer<buffer_len>& m): mb(m), index(m.read_index()) {}

  inline void skip(size_t s) {
    message_buffer<buffer_len>::advance_index(index, s);
  }
  ...
  inline bool read(char* d, size_t s) {
    ...
    throw_datastream_range_error("peek", mb.bytes_to_read_from_index(index), s - mb.bytes_to_read_from_index(index));
  }
