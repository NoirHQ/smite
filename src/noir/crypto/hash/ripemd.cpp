...
crypto::hash<ripemd160>& ripemd160::update(std::span<const char> in) {
  ...
  if (bufsize && bufsize + len >= 64) {
    ...
    bytes += 64 - bufsize;
    data += 64 - bufsize;
    impl::Transform(s, buf);
    bufsize = 0;
  }
  while (end - data >= 64) {
    // Process full chunks directly from the source.
    impl::Transform(s, data);
    bytes += 64;
    data += 64;
  }
  if (end > data) {
...
void ripemd160::final(std::span<char> out) {
  ...
  update(std::span(reinterpret_cast<const char*>(pad), 1 + ((119 - (bytes % 64)) % 64)));
  update(std::span(reinterpret_cast<const char*>(sizedesc), 8));
  WriteLE32(hash, s[0]);
