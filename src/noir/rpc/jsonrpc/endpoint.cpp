...
response detail::endpoint_impl::rpc(const fc::variant& message) {
  ...
  try {
    ...
    // This second layer try/catch is to isolate errors that occur after parsing the id so that the id is properly
    // returned.
    try {
      if (!response.error)
      ...
      response.error =
        error(error_code::server_error, "Unknown error - parsing rpc message failed", fc::variant(e.what()));
    } catch (...) {
...
fc::variant detail::endpoint_impl::handle_request(const std::string& message) {
  try {
    ...
    response response;
    response.error = error(error_code::server_error, "Unknown exception",
      fc::variant(fc::unhandled_exception(FC_LOG_MESSAGE(warn, "Unknown Exception"), std::current_exception())
                    .to_detail_string()));
    return response;
  }
}
