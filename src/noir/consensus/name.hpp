...
namespace fc {
class variant;
void to_variant(const noir::consensus::name& c, fc::variant& v);
void from_variant(const fc::variant& v, noir::consensus::name& check);
} // namespace fc
...
constexpr uint64_t char_to_symbol(char c) {
  ...
  if (c >= '1' && c <= '5')
    ...
    throw std::runtime_error(fmt::format("Name contains invalid character: ({})", std::string(1, c)));
...
constexpr uint64_t string_to_uint64_t(std::string_view str) {
  //  EOS_ASSERT(str.size() <= 13, name_type_exception, "Name is longer than 13 characters (${name}) ", ("name",
  //  std::string(str)));
...
private:
  ...
  friend void fc::from_variant(const fc::variant& v, noir::consensus::name& check);
...
public:
  ...
  friend constexpr bool operator==(const name& a, uint64_t b) {
    return a.value == b;
  }
  friend constexpr bool operator!=(const name& a, uint64_t b) {
    return a.value != b;
...
namespace std {
template<>
struct hash<noir::consensus::name> : private hash<uint64_t> {
  typedef noir::consensus::name argument_type;
...
FC_REFLECT(noir::consensus::name, (value))
