...
struct light_block {
  ...
  Result<void> validate_basic(std::string chain_id) {
    if (!s_header)
      return Error::format("missing signed header");
    if (!val_set)
      return Error::format("missing validator set");
    if (auto ok = s_header->validate_basic(chain_id); !ok)
      return Error::format("invalid signed header: {}", ok.error());
    if (auto ok = val_set->validate_basic(); !ok)
      return Error::format("invalid validator set: {}", ok.error());
    if (auto val_set_hash = val_set->get_hash(); val_set_hash != s_header->header->validators_hash)
      return Error::format("miss match of hash between validator set hash");
    return success();
  }
