...
class PeerUpdates {
public:
  noir::Chan<std::shared_ptr<PeerUpdate>> router_updates_ch;
  noir::Chan<std::shared_ptr<PeerUpdate>> reactor_updates_ch;
};
...
public:
  ...
  void process_peer_event(noir::Chan<std::monostate>& done, const std::shared_ptr<PeerUpdate>& pu);
  auto accepted(const NodeId& peer_id) -> boost::asio::awaitable<noir::Result<void>>;
  auto ready(noir::Chan<std::monostate>& done, const NodeId& peer_id, ChannelIdSet& channels)
    -> boost::asio::awaitable<void>;
  auto dial_next(noir::Chan<std::monostate>& done)
  ...
  auto dial_failed(noir::Chan<std::monostate>& done, const std::shared_ptr<NodeAddress>& address)
  ...
  auto broadcast(noir::Chan<std::monostate>& done, const std::shared_ptr<PeerUpdate>& peer_update)
    -> boost::asio::awaitable<void>;
  ...
  auto disconnected(Chan<std::monostate>& done, const NodeId& peer_id) -> boost::asio::awaitable<void>;
