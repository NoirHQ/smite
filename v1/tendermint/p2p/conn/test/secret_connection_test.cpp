...
TEST_CASE("SecretConnection: openssl - hkdf", "[tendermint][p2p][conn]") {
  ...
  *p++ = OSSL_PARAM_construct_octet_string("info",
    (void*)"TENDERMINT_SecretConnection_KEY_AND_CHALLENGE_GEN",
  ...
  *p = OSSL_PARAM_construct_end();
  ...
  // std::cout << "key1_hex = " << to_hex(std::span((const ByteType*)key, 32)) << std::endl;
  // std::cout << "key2_hex = " << to_hex(std::span((const ByteType*)(key+32), 32)) << std::endl;
  // std::cout << "challenge_hex = " << to_hex(std::span((const ByteType*)(key+64), 32)) << std::endl;
  CHECK(
    to_hex(std::span((const ByteType*)key, 32)) == "80a83ad6afcb6f8175192e41973aed31dd75e3c106f813d986d9567a4865eb2f");
  CHECK(to_hex(std::span((const ByteType*)(key + 32), 32)) ==
    "96362a04f628a0666d9866147326898bb0847b8db8680263ad19e6336d4eed9e");
  CHECK(to_hex(std::span((const ByteType*)(key + 64), 32)) ==
    "2632c3fd20f456c5383ed16aa1d56dc7875a2b0fc0d5ff053c3ada8934098c69");
}
...
TEST_CASE("SecretConnection: libsodium - chacha20", "[tendermint][p2p][conn]") {
  ...
  crypto_stream_chacha20_ietf_xor(ciphertext,
    reinterpret_cast<const unsigned char*>(msg.data()),
    msg.size(),
    nonce,
    reinterpret_cast<const unsigned char*>(key.data()));
  crypto_stream_chacha20_ietf_xor(decrypted,
