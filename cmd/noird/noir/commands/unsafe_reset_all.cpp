  ...
  auto cmd =
    root
      ...
      ->final_callback([]() {
        ...
        ilog("resetting ${home_dir}/${path}", ("home_dir", home_dir.string())("path", priv_val_state_path));
...
bool remove_dir(fs::path target_dir) {
  if (fs::exists(target_dir)) {
    if (!fs::is_directory(target_dir)) {
      elog("unable to delete ${dir}; it's not a directory", ("dir", target_dir.string()));
      return false;
    }
  }
  ilog("deleting directory ${dir}", ("dir", target_dir.string()));
